...
const HamburgerIcon = () => (
  ...
  >
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M14.5827 5.83337H5.83268C5.05913 5.83337 4.31727 6.14066 3.77029 6.68765C3.22331 7.23463 2.91602 7.97649 2.91602 8.75004V26.25C2.91602 27.0236 3.22331 27.7655 3.77029 28.3124C4.31727 28.8594 5.05913 29.1667 5.83268 29.1667H14.5827V5.83337ZM6.38539 13.5523C6.112 13.8258 5.95841 14.1967 5.95841 14.5834C5.95841 14.9701 6.112 15.3409 6.38539 15.6144L8.27102 17.5L6.38539 19.3857C6.24611 19.5202 6.13501 19.6811 6.05858 19.859C5.98215 20.037 5.94192 20.2283 5.94023 20.422C5.93855 20.6156 5.97545 20.8076 6.04878 20.9868C6.1221 21.1661 6.23039 21.3289 6.36731 21.4658C6.50424 21.6028 6.66707 21.711 6.84629 21.7844C7.02552 21.8577 7.21755 21.8946 7.41118 21.8929C7.60482 21.8912 7.79618 21.851 7.97411 21.7746C8.15203 21.6981 8.31295 21.587 8.44747 21.4477L11.3641 18.5311C11.6375 18.2576 11.7911 17.8867 11.7911 17.5C11.7911 17.1133 11.6375 16.7425 11.3641 16.469L8.44747 13.5523C8.174 13.2789 7.80313 13.1254 7.41643 13.1254C7.02973 13.1254 6.65887 13.2789 6.38539 13.5523Z"
      fill="#003366"
    />
    <path
      d="M17.5 29.1667H29.1667C29.9402 29.1667 30.6821 28.8594 31.2291 28.3124C31.776 27.7655 32.0833 27.0236 32.0833 26.25V8.75004C32.0833 7.97649 31.776 7.23463 31.2291 6.68765C30.6821 6.14066 29.9402 5.83337 29.1667 5.83337H17.5V29.1667Z"
      fill="#003366"
    />
  </svg>
);
...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, `users/${userProfile.uid}/chats`),
      orderBy("timestamp", "desc")
    );
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const history = snapshot.docs.map((doc) => ({
        id: doc.id,
        title: doc.data().prompt.substring(0, 30) + "...",
      }));
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "ai", content: `Maaf, terjadi kesalahan: ${err.message}` },
      ]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDeleteChat = async (chatId) => {
    ...
    try {
      await deleteDoc(doc(db, "users", userProfile.uid, "chats", chatId));
      console.log(`Chat dengan ID ${chatId} berhasil dihapus.`);
    ...
    <AuthGuard>
      <div className="flex h-screen bg-white font-sans">
        ...
        <aside className="hidden md:flex flex-col w-64 bg-gray-50 shrink-0">
          ...
          <div className="p-2">
            <button
              ...
              className="flex items-center gap-3 w-full text-left p-3 rounded-lg hover:bg-gray-200 font-medium text-sm"
            >
              <HomeIcon /> Beranda
            </button>
          </div>
          ...
          <nav className="flex-1 p-2 space-y-1 overflow-y-auto">
            <p className="px-3 pt-4 pb-2 text-xs font-semibold text-gray-500 uppercase justify-start flex">
              Riwayat
            </p>
            {chatHistory.map((chat) => (
              ...
              >
                <button
                  ...
                  className={`w-full text-left block p-3 rounded-lg text-sm truncate pr-10 ${
                ...
                <div className="absolute top-2 right-3">
                  <Menu as="div" className="relative inline-block text-left">
                    ...
                    <Menu.Items className="absolute right-0 mt-1 w-32 origin-top-right bg-white border border-gray-200 rounded-md shadow-lg focus:outline-none z-50">
                      <div className="py-1">
                        <Menu.Item>
                          {({ active }) => (
                            <button
          ...
          {mobileMenuOpen && (
            ...
            >
              <motion.div
                ...
                className="absolute top-0 left-0 h-full w-4/5 max-w-xs bg-gray-50 shadow-xl"
              ...
              >
                ...
                >
                  ...
                  <motion.button
                    ...
                    className="flex mb-5 items-center gap-3 w-full text-left p-3 rounded-lg hover:bg-gray-200 font-medium text-sm"
                  >
                    <NewChatIcon /> Percakapan Baru
                  </motion.button>
                  ...
                  <motion.p
                    ...
                    className="px-3 pt-2 pb-2 text-xs font-semibold text-gray-500 uppercase text-left"
                  >
                    Percakapan
                  </motion.p>
        ...
        <main className="flex-1 flex flex-col shadow-lg h-screen">
          ...
          <header className="md:hidden flex fixed top-0 w-full items-center p-4 bg-white">
            <button onClick={() => setMobileMenuOpen(true)} className="p-2">
              <HamburgerIcon />
            </button>
          </header>
          <div className="flex-1 p-4 md:pt-0 pt-24 sm:p-6 lg:p-8 overflow-y-auto bg-white">
              ...
              {messages.length === 0 && !isLoading && (
                <div className="text-left py-20 flex flex-row gap-2 md:gap-10">
                  ...
                  <h2 className="mt-4 text-xl md:text-2xl font-semibold text-gray-800">
                    Halo! Lagi pengen liburan? Yuk cari tempat seru bareng aku!
                  </h2>
                </div>
              )}
